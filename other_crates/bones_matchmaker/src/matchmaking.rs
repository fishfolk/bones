...
pub async fn handle_request_matchaking(
    ...
    for _ in 0..200 {
        let room_is_full = state.matchmaking_rooms.get(&match_info)
    ...
    let can_join = state.matchmaking_rooms.get(&match_info)
        .map(|room| room.get().len() < match_info.max_players as usize)
        .unwrap_or(true);
    ...
    let new_player_count = state.matchmaking_rooms.update(&match_info, |_, members| {
...
async fn send_matchmaking_updates(match_info: &MatchInfo, new_player_count: u32) -> Result<Vec<Connection>> {
    ...
    // Update stored connections
    {
        ...
        if let Err(e) = state.matchmaking_rooms.insert(match_info.clone(), active_connections.clone()) {
            warn!("Failed to insert updated matchmaking room: {:?}. Error: {:?}", &match_info, e);
        }
    }
...
async fn start_matchmaked_game_if_ready(ep: Endpoint, match_info: &MatchInfo) -> Result<()> {
    let members = {
        let state = MATCHMAKER_STATE.lock().await;
        state.matchmaking_rooms.remove(match_info)
